		    ...
		    <div class="layui-tab-item layui-show">
				<form class="layui-form layui-form-pane1" action="" onsubmit="javascript:return false;">
				  ...
				  <div style="padding: 15px;">
					...
					<button class="layui-btn layui-btn-primary" id={{.Id}}><i class="layui-icon">&#xe654;</i></button>				
					{{end}}
					{{range .maps_rd}}
					{{end}}
	...
	$('#{{.Id}}').on('click',function(){
		...
		layer.open({
		 ...
		 // offset: 'rb', //右下角弹出
		  ...
		  maxmin: true,
		  anim: 2,
		  content: ['/v1/restaurant_ready/addreadydish?id='+rid+'&mname={{.Name}}'], //iframe的url，no代表不显示滚动条
		  cancel: function(index, layero){ 
			  if(confirm('确定要关闭么')){ //只有当点击confirm框的确定时，该层才会关闭
			    layer.close(index)
				window.location.reload();				
